...
public class KeyDataCreator {
    ...
    public static ArrayList<RegistroHashExtensivel> criarParesHE(String path) {
        ArrayList<RegistroHashExtensivel> registrosHE = new ArrayList<RegistroHashExtensivel>();
        try {
            RandomAccessFile arq = new RandomAccessFile(path, "rw");
            ...
            while(arq.getFilePointer() < arq.length()) {
                ...
                // Adiciona o registro à lista
                if (alive == 1) { // Verifica se o registro está "vivo"
                    registrosHE.add(new RegistroHashExtensivel(jobId, currentOffset));
                }
            }
            arq.close();
        } catch (Exception e){
